...
local function find_help_window_iter()
	for win_handle in iter(api.nvim_list_wins()) do
		...
		if buf_handle and buf_handle > 0 then
			local success, bt = pcall(api.nvim_buf_get_option, buf_handle, "buftype")
			if success and bt == "help" then
				return win_handle
			end
		end
	end
...
keymap.set("n", "<leader>ho", function()
	local help_win = find_help_window_iter()
